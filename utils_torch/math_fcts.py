...
def AAcceleration(act_R, act_U):
    if type(act_R).__module__ == 'numpy':
        return AAcceleration_numpy(act_R, act_U)
    elif type(act_R).__module__ == 'torch':
        return AAcceleration_torch(act_R, act_U)


def AAcceleration_torch(act_R, act_U):
    device = act_R.device
    
    act_R = torch.moveaxis(act_R, -1, 0)
    act_U = torch.moveaxis(act_U, -1, 0)
    
    nd = act_R.shape[0]
    
    A = torch.zeros((nd,nd)).to(device) #TODO: improve this with symmetry!!!
    for j0 in range(nd):
        for j1 in range(nd):
            tmp = torch.sum( (act_R[j0]*act_R[j1]).flatten() )
            A[j0,j1] = tmp
            A[j1,j0] = tmp
    Ainv = torch.linalg.pinv(A)
    
    s = torch.sum(Ainv,axis=0) / torch.sum(Ainv.flatten())
    
    U = torch.zeros(act_U.shape[1:], dtype=act_U.dtype).to(device)
    for i0 in range(nd):
        U += s[i0] * (act_U[i0]-act_R[i0])

    return U

def AAcceleration_numpy(act_R, act_U):
